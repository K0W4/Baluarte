...
**The brand red does two jobs, so it is two tokens.** `#C72B2B` comes from the Order's colours and never varies by appearance, but on a dark card it reads at 3,09:1 — under the 4,5:1 floor for normal text — and lightening it would drop the white sitting on top of it. So the split is by function, not by mode:

- `Theme.accent` — the brand itself, and the default for anything **drawn**: a primary button's background, the selected day's circle, and every icon — chevrons, pluses and xmarks included. `Theme.onAccent` (white) on it is 5,51:1 in both modes. It reads `Color("AccentColor")` on purpose: `Color.accentColor` is not the catalogue colour, it is whatever `tint` is in the environment, and using it made every fill follow the tint.
- `Theme.accentText` — **text only**, and only where the brand has to be read as a run of words: `#C72B2B` in light, lighter in dark, because `#C72B2B` on a dark card is 3,09:1 and fails the 4,5:1 floor for normal text. It is deliberately **not** for icons and **not** for the global `.tint()`: a glyph answers to the 3:1 component floor, not the text floor, and everything the tint reaches — the tab bar, switches, progress bars, picker values — is drawn, not read. The global tint is `accent`, in `ContentView`. Package 15 migrated 33 thin glyphs to `accentText` on the reasoning that a chevron is "thin like text"; that was wrong and was reverted. If you are colouring an `Image`, the answer is `accent`. Today `accentText` has four call sites, all of them running text.

**Destructive is separated by form, never by hue.** `Theme.accent` and `Theme.destructive` contrast only 1,62:1 with each other — as long as both were the same shape, colour decided nothing. The primary button is filled; the destructive one is an outline on a transparent ground. A filled destructive button would put destruction back to looking like the action the app wants you to take.
...
**Invite codes are the fast path in.** `chapter_invite` codes are generated by a column `DEFAULT` — `code` is left out of the `INSERT` grant, so a client cannot choose its own; `SELECT` is granted at table level, so what keeps codes unreadable is the absence of a policy, not a column grant — redeemed through the `redeem_chapter_invite` RPC, and always grant `member` — a code travelling through a WhatsApp group must never hand out administration. There is deliberately **no RLS policy** letting non-members read `chapter_invite`, so codes cannot be enumerated, and redemption is rate limited through `invite_attempt` because the whole model rests on the code being unguessable. `DeepLink` (`baluarte://invite/<code>`) is a custom URL scheme, not a Universal Link: there is no domain to host an AASA file, and the code is always typable by hand. The same scheme carries password recovery (`baluarte://password-recovery`), which is why `sendPasswordReset` passes an explicit `redirectTo` — without one Supabase falls back to the project's Site URL, and there is no site. `SetNewPasswordView` is a sheet over every route on purpose: the link can be opened while signed out, inside the app, or waiting in the approval queue.
...
**When an operation needs an RPC**: only when the caller must legitimately touch rows they cannot see under RLS, or when a multi-row invariant must be atomic (e.g. `set_event_attendance` — a `with check` cannot inspect an array delta). Everything else stays plain PostgREST plus a policy. Authorization failures raise `42501` (→ 403); business rules raise `23514` (→ 400); "not found" raises `PT404`, because PostgREST has no mapping for `P0002` and answers it with a 500 — `PTxyz` is its documented way for a `raise` to choose its own status.
...
**Authorisation comes before any detail of the target — including whether it exists.** Looking the row up first and raising `PT404` tells a stranger which ids are real: a made-up id answers "not found", a real one answers "no permission". This has been introduced and fixed twice (`set_membership_access_level`, `review_chapter_request`); the second time `supabase/tests/rls.sh` caught it before it reached `dev`, which is what those probes are for.
