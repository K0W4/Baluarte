...
- `Theme.accent` — the fill and the large glyph: a primary button's background, the selected day's circle, an icon. `Theme.onAccent` (white) on it is 5,51:1 in both modes. It reads `Color("AccentColor")` on purpose: `Color.accentColor` is not the catalogue colour, it is whatever `tint` is in the environment, and using it made every fill follow the tint.
- `Theme.accentText` — the same brand where it has to be *read*: accent-coloured text and thin icons. `#C72B2B` in light, lighter in dark. **This is what the app's global `.tint()` uses**, so a new `Button` inherits the legible colour by default and the brand fill is the exception someone writes deliberately.
