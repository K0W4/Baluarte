...
- `Theme.accentText` — **text only**, and only where the brand has to be read as a run of words: `#C72B2B` in light, lighter in dark, because `#C72B2B` on a dark card is 3,09:1 and fails the 4,5:1 floor for normal text. It is what the app's global `.tint()` uses. It is deliberately **not** for icons: a glyph answers to the 3:1 component floor, not the text floor, and the Order's red is the icon colour. Package 15 migrated 33 thin glyphs to `accentText` on the reasoning that a chevron is "thin like text" — that was wrong and was reverted. If you are colouring an `Image`, the answer is `accent`.
