-- Mensagens do servidor em três idiomas.
--
-- Até aqui todo `raise` carregava apenas uma frase em português, e o cliente a exibia
-- verbatim -- então quem usa o aparelho em inglês ou espanhol recebia português. Traduzir
-- no servidor seria pior: o Postgres não conhece o idioma de quem chamou, e a mensagem
-- também vai para o log e para quem depura por curl.
--
-- Cada `raise` que pode chegar a uma pessoa passa a carregar um `hint` estável no formato
-- `baluarte.<chave>`, opcionalmente seguido de `:<argumento>`. O PostgREST repassa o HINT
-- do Postgres no campo `hint` do corpo do erro, e é ele que o cliente traduz. A frase em
-- português continua sendo a mensagem da exceção de propósito: é o que aparece no log, no
-- curl e em versões do app anteriores a esta.
--
-- Os `raise` que não ganham hint são os `insufficient_privilege` puros: não têm motivo
-- específico a comunicar e continuam virando uma negativa genérica no cliente.
--
-- Ficam de fora os `raise` de tempo de migration (backfill, purga de placeholders,
-- checagem de duplicatas do registro), que nunca alcançam um usuário.
--
-- As funções são reescritas inteiras porque o Postgres não permite remendar um `raise`
-- isolado. Cada uma reproduz a definição vigente -- a mais recente, não a da migration
-- que a criou -- somando apenas o hint.
...
begin
  ...
  if not found then
    raise exception 'Evento não encontrado.'
      using errcode = 'P0002', hint = 'baluarte.event_not_found';
  end if;
...
begin
  ...
  if not found then
    raise exception 'Solicitação não encontrada.'
      using errcode = 'P0002', hint = 'baluarte.request_not_found';
  end if;
...
begin
  ...
  if v_level is null then
    raise exception 'Você não participa deste Capítulo.'
      using errcode = 'P0002', hint = 'baluarte.not_in_chapter';
  end if;
...
begin
  ...
  if not found then
    raise exception 'Vínculo não encontrado.'
      using errcode = 'P0002', hint = 'baluarte.membership_not_found';
  end if;
